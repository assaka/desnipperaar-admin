<!DOCTYPE html>
@php $en = ($locale ?? 'nl') === 'en'; @endphp
<html lang="{{ $en ? 'en' : 'nl' }}">
<head>
<meta charset="UTF-8">
<title>{{ $certificate->certificate_number }} — {{ $en ? 'Certificate of destruction' : 'Vernietigingscertificaat' }}</title>
<style>
    ...
    <table class="signs">
        <tr>
            <td>
                <div class="sig-box">
                    <h4>{{ $L['operator'] }}</h4>
                    <div class="sub">{{ $L['name'] }}: {{ $certificate->operator_name ?? '' }}</div>
                    <div class="sub">{{ $L['date'] }}: {{ $certificate->destroyed_at?->format('d-m-Y') ?? '' }}</div>
                    <div class="sub">{{ $L['signature'] }}:</div>
            ...
            <td>
                <div class="sig-box">
                    <h4>{{ $L['witness'] }}</h4>
                    <div class="sub">{{ $L['name'] }}: {{ $order->customer_name }}</div>
                    <div class="sub">{{ $L['date'] }}: {{ $bon?->picked_up_at?->format('d-m-Y') ?? '' }}</div>
                    <div class="sub">{{ $L['signature'] }}:</div>
