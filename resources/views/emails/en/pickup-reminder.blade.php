@component('emails.en._layout', ['title' => 'Pickup reminder '.$order->order_number])
<h1 style="font-size:22px;font-weight:900;margin:0 0 12px;">We collect tomorrow.</h1>

<p>Dear {{ explode(' ', $order->customer_name)[0] }},</p>

<p>A short reminder. Tomorrow, {{ $order->pickup_date->format('d-m-Y') }}, we collect your sealed roll container.</p>

@if ($shifted)
<p style="background:#FFF8E1;border-left:4px solid #F5C518;padding:12px 14px;">
    Please note. We normally come on {{ $order->subscription_scheduled_for->format('d-m-Y') }}, but that day is a public holiday or weekend this time. So we come on {{ $order->pickup_date->format('d-m-Y') }} instead. Your fixed pickup day does not change.
</p>
@endif

<table cellpadding="6" style="border-collapse:collapse;font-size:14px;margin:16px 0;">
    <tr><td style="background:#F5F5F5;font-weight:700;">Pickup date</td><td><strong>{{ $order->pickup_date->format('d-m-Y') }}</strong></td></tr>
