@component('emails._layout', ['title' => 'Deelnemer afgemeld in je groepsdeal'])
<h1 style="font-size:22px;font-weight:900;margin:0 0 12px;">Een deelnemer heeft afgemeld</h1>

<p>Beste {{ explode(' ', $organizer->customer_name)[0] }},</p>

<p>Een deelnemer in je groepsdeal in <strong>{{ $deal->city }}</strong> op <strong>{{ $deal->pickup_date->locale('nl')->translatedFormat('l j F Y') }}</strong> heeft hun inschrijving geannuleerd.</p>
...
<table cellpadding="6" cellspacing="0" border="0" style="border-collapse:collapse;font-size:14px;margin-bottom:16px;">
  ...
  <tr><td style="color:#666;">Bracht:</td><td><strong>{{ $leftBoxCount }} dozen</strong>@if ($leftContainerCount > 0) en <strong>{{ $leftContainerCount }} rolcontainers</strong>@endif</td></tr>
</table>
...
<p style="font-size:12px;color:#777;">Wil je nieuwe deelnemers werven? Deel de publieke link: <a href="https://desnipperaar.nl/groepsdeals/{{ $deal->slug }}" style="color:#777;">desnipperaar.nl/groepsdeals/{{ $deal->slug }}</a>.</p>
