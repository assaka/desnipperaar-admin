@component('emails._layout', ['title' => 'Groepsdeal niet doorgegaan'])
<h1 style="font-size:22px;font-weight:900;margin:0 0 12px;">Je groepsdeal-aanvraag</h1>

@if ($deal->organizerParticipant)
<p>Beste {{ explode(' ', $deal->organizerParticipant->customer_name)[0] }},</p>
@endif

<p>Helaas kunnen wij je groepsdeal-aanvraag voor <strong>{{ $deal->city }}</strong> op <strong>{{ $deal->pickup_date->locale('nl')->translatedFormat('l j F Y') }}</strong> nu niet inplannen.</p>
