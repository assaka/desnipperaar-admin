@component('emails.es._layout', ['title' => 'Recordatorio de recogida '.$order->order_number])
...
<p>Un breve recordatorio. Mañana, {{ $order->pickup_date->format('d-m-Y') }}, recogemos su contenedor con ruedas precintado.</p>

@if ($shifted)
<p style="background:#FFF8E1;border-left:4px solid #F5C518;padding:12px 14px;">
    Tenga en cuenta. Normalmente venimos el {{ $order->subscription_scheduled_for->format('d-m-Y') }}, pero ese día es esta vez festivo o fin de semana. Por eso vamos el {{ $order->pickup_date->format('d-m-Y') }}. Su día de recogida fijo no cambia.
</p>
@endif

<table cellpadding="6" style="border-collapse:collapse;font-size:14px;margin:16px 0;">
    <tr><td style="background:#F5F5F5;font-weight:700;">Fecha de recogida</td><td><strong>{{ $order->pickup_date->format('d-m-Y') }}</strong></td></tr>
