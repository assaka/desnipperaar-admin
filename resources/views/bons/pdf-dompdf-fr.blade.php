...
<div class="wrap">
    @php $isBezorg = $bon->mode === 'bezorging'; $isRetour = $bon->mode === 'retour'; $isOphaal = ! $isBezorg && ! $isRetour; $titel = ['ophaal' => "Bon d'enlèvement", 'breng' => 'Bon de dépôt', 'mobiel' => 'Bon mobile', 'bezorging' => 'Bon de livraison', 'retour' => 'Bon de retour'][$bon->mode] ?? 'Bon'; $kolomKop = $isBezorg ? 'Livraison' : ($isRetour ? 'Retour' : 'Enlèvement'); @endphp
    <h1>{{ $titel }}</h1>
        ...
        <tr>
            <td class="meta-col">
                ...
                @if ($ophaalAdres['address'])
                    <div class="row"><span class="k">{{ $bon->order->hasSeparatePickupAddress() ? "Adresse d'enlèvement" : "Adresse" }}</span><span class="v">{{ $ophaalAdres['address'] }}</span></div>
                @endif
                <div class="row"><span class="k">Code postal</span><span class="v"><span style="font-family:'Courier New',monospace;">{{ $ophaalAdres['postcode'] }}</span> {{ $ophaalAdres['city'] }}</span></div>
