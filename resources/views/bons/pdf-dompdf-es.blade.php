        ...
        <tr>
            <td class="meta-col">
                ...
                @if ($ophaalAdres['address'])
                    <div class="row"><span class="k">{{ $bon->order->hasSeparatePickupAddress() ? 'Dirección de recogida' : 'Dirección' }}</span><span class="v">{{ $ophaalAdres['address'] }}</span></div>
                @endif
                <div class="row"><span class="k">Código postal</span><span class="v"><span style="font-family:'Courier New',monospace;">{{ $ophaalAdres['postcode'] }}</span> {{ $ophaalAdres['city'] }}</span></div>
                ...
                <div class="sig-box">
                    ...
                    @elseif ($bon->handtekeningKwijtgescholden())
                        ...
                        <div style="font-size:8pt;text-align:left;">
                            <strong style="text-transform:uppercase;">El cliente no estaba en condiciones de firmar</strong><br>
