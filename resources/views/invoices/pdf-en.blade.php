...
<div class="wrap">
    ...
    @php
        ...
        // A coupon is a line with a negative amount, so it needs no discount row of
        // its own; qty and unit price would only repeat the amount.
    ...
    @endphp
        <tbody>
            @foreach ($invoice->lines as $line)
                <tr>
                    <td>@if ($isCoupon($line)){{ 'Discount code '.($line['code'] ?? '') }}@if (!empty($line['pct']))<span style="color:#777;">&nbsp;({{ \App\Support\Pricing::formatPercentage($line['pct']) }}% × € {{ number_format($line['base'], 2, ',', '.') }})</span>@endif@else{{ $tr($line['label']) }}@endif@if ($isStaffel($line))<span style="color:#2E7D32;font-weight:700;">&nbsp;*</span>@endif</td>
