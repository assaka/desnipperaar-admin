...
<head>
<meta charset="UTF-8">
<title>{{ $invoice->isCreditNote() ? "Abono " : "Factura " }}{{ $invoice->invoice_number }}</title>
<style>
...
<div class="wrap">
    ...
    <table class="top">
        <tr>
            <td class="leverancier">
                ...
                <div>{{ $co['country'] }}</div>
                @if ($co['kvk']) <div style="margin-top:2mm;">Reg. merc.: <strong>{{ $co['kvk'] }}</strong></div> @endif
                @if ($co['btw']) <div>IVA: <strong>{{ $co['btw'] }}</strong></div> @endif
        ...
        <div class="klant">
            ...
            <td class="doc-info">
                <h1>{{ $invoice->isCreditNote() ? "ABONO" : "FACTURA" }}</h1>
    ...
    <table class="totals">
        ...
        @endif
        @if ($couponAmount > 0)
            <tr><td class="k">Código de descuento {{ $couponLine['code'] ?? '' }}@if (!empty($couponLine['pct'])) <span style="color:#777;font-size:9pt;white-space:nowrap;">({{ \App\Support\Pricing::formatPercentage($couponLine['pct']) }}% × € {{ number_format($couponLine['base'], 2, ',', '.') }})</span>@endif</td><td class="v">- € {{ number_format($couponAmount, 2, ',', '.') }}</td></tr>
        @endif
    ...
    <div class="pay">
        <h3>Pago</h3>
        <div class="row"><span class="k">Vencimiento</span><span class="v">{{ $invoice->due_at->format('d-m-Y') }} ({{ config('desnipperaar.invoice.payment_terms_days') }} días)</span></div>
        <div class="row"><span class="k">Importe</span><span class="v">€ {{ number_format($invoice->amount_incl_btw, 2, ',', '.') }}</span></div>
        ...
        <div class="row"><span class="k">Referencia</span><span class="v">{{ $invoice->invoice_number }}</span></div>
    </div>
    <p class="small">Por favor, pague indicando el número de factura. ¿Preguntas? {{ $co['email'] }}.</p>

</div>

<div class="foot">RGPD · DIN 66399 · Personal con VOG · Asegurado · Cobertura € 2,5M</div>
