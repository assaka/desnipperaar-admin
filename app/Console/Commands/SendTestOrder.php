...
class SendTestOrder extends Command
{
    ...
    public function handle(): int
    {
        ...
        if ($reschedule) {
            // De klant plant en verzet zijn moment tegenwoordig zelf op de
            // planpagina, dus wat er te testen valt is tweemaal dezelfde
            // bevestiging: eerst het gekozen moment, dan het verzette.
            $order->update([
                'state'         => Order::STATE_BEVESTIGD,
                'pickup_date'   => now()->addDays(3)->toDateString(),
                'pickup_window' => '10:00-11:00',
            ...
            $previous = $order->pickup_date->format('d-m-Y').' '.$order->pickup_window;
            $order->update([
                'pickup_date'                   => now()->addDays(7)->toDateString(),
                'pickup_window'                 => '13:00-14:00',
