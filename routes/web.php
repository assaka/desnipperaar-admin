...
// The actual opt-out. Serves both the confirm button on the page above and the
// RFC 8058 one-click POST from a mailbox provider. Neither carries a session of
// ours, so the token is the authenticator and the route is CSRF-exempt (see
// bootstrap/app.php).
Route::post('/afmelden/{token}', [\App\Http\Controllers\UnsubscribeController::class, 'unsubscribe'])
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::post('/orders/{order}/reset-subscription', [\App\Http\Controllers\OrderController::class, 'resetSubscription'])->name('orders.reset-subscription');
    ...
    Route::post('/orders/{order}/send-plan-link', [\App\Http\Controllers\OrderController::class, 'sendPlanLink'])->name('orders.send-plan-link');
    Route::post('/orders/{order}/ask-review', [\App\Http\Controllers\OrderController::class, 'askReview'])->name('orders.ask-review');
